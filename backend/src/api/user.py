...
async def edit_profile(user_data: UserEdit, session: AsyncSession = Depends(create_session)):
    await user_service.update_user(session=session,user_name=user_data.user_name,  new_classe= user_data.classe, new_user_name = user_data.new_user_name)
    user =await  user_service.get_user_by_user_name(user_name= user_data.user_name, session=session)   
    return user
...
async def create_user(
    ...
    user_exist = await user_service.user_exist(user_name=user_data.user_name, session=session)
    if user_exist:
        raise HTTPException(
    ...
    number_contribution =  await user_service.get_number_contribution(session=session,user_id= new_user.id)
...
async def login_user(login_data: UserLogin, session: AsyncSession = Depends(create_session)):
    ...
    user =  await user_service.get_user_by_user_name(user_name=user_name, session=session)
    ...
    if user:
        ...
        if passwd_valid:
            ...
            number_contribution = str( await user_service.get_number_contribution(session=session,user_id= user.id))
            return JSONResponse(
                content= {
